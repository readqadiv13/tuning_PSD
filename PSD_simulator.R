...
#   source (file.path (poolDir, 'PSD_archive.R'), chdir = F)
    ...
    tunePSD  <-  function (...) {
            eng  <-  ifelse (Lang == '英語', T, F)
            use.D50  <-  ifelse (D50 == 'マイクロトラック測定値', T, F)
            ##  First preparation
            ...
            tag  <-  function (dt)  if (! alive (dt))  NULL else  paste0 ('_', dt $ 'ロット番号')
            ...
            rec  <-  rep (NA, 15)  %>%  t (.)  %>% {skipMess. (as_tibble (.))}  %>%  mutate_at (1, as_datetime, tz = 'Asia/Tokyo')  %>%    #  Vacant tibble for recording
                        setNames (c ('計算日', map. (c ('砥粒種', '粒度', 'ロット番号', '配合率'), ~ paste0 (., c ('A', 'B', 'C'))), 'D50誤差率', '右曲部面積の誤差率' ) )
            rec [1, 1]  <-  now ()
